...
// Restore pro mode if returning from PayPal Pro payment
(function() {
  ...
  // Also restore if they had pro active this session
  if (localStorage.getItem('qs_pro') === 'true') {
    enableProFeatures();
  }
})();
...
function downloadPDF(watermarked = false) {
  ...
  doc.text('Generated by QuickScope · quickscope.vercel.app · This document is legally binding upon signature by both parties.', W/2, H - 5, {align:'center'});
...
function exportCSV() {
  ...
  // Trigger PDF download if redirected from PayPal $4 payment
    ...
    if (urlParams.get('paid') === 'true') {
      ...
      if (saved) {
        try {
          ...
          localStorage.removeItem('qs_pending_download');
          setTimeout(() => downloadPDF(false), 800);
        } catch(e) { console.error('Could not restore form data', e); }
      } else {
        setTimeout(() => downloadPDF(false), 800);
      }
      history.replaceState({}, '', window.location.pathname);
